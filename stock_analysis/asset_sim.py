...
time_step = 252
N = 1000
obj = BS_sim(r = 0.02, sigma = 0.5, g = 0.01, S_0 = 400, T = 1, t = 0, time_step = time_step, N = N)
# print(obj.exact().mean(axis = 0))
a = obj.exact()
b = [x[-1]/x[0]-1 for x in a]
y = np.array([[0, 1] if x >= 0 else [1, 0] for x in b])
y = torch.Tensor(y)
...
for m in range(0, N):
    ...
    for i in range(1, time_step + 1):
        b[i][i:] = [y-x for x, y in zip(a[m][:-1], a[m][i:])]

    cutoff = math.ceil((time_step + 1)/2)
    c = np.empty((cutoff, cutoff,))
    c[:] = np.nan
    for i in range(0, cutoff):
        c[i] = b[i][-cutoff:]

    # x is the square part of the upper triangular matrix 
    x = torch.Tensor(np.array(c)).view(-1, 1, cutoff, cutoff)
...
if __name__ == "__main__":
    ...
    run_optim(alpha=0.005, size=cutoff, X_y_set=X_y_set).model_test()
